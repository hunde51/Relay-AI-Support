import type { TicketStatus, TicketPriority, TicketCategory } from "@/data/mockTickets";
...
const categoryStyles: Record<TicketCategory, string> = {
  ...
  feature: "bg-chart-2/15 text-chart-2",
};

export function CategoryBadge({ category }: { category: TicketCategory }) {
  return (
    <span className={cn("inline-flex rounded-md px-2 py-0.5 text-[11px] font-medium capitalize", categoryStyles[category])}>
      {category}
    </span>
  );
}
